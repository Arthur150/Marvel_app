    ...
    fun loadComics() {
        viewModelScope.launch {
            val jsonResponse = Gson().fromJson<JsonResponse<MarvelComic>>(
                Gson().toJson(GetMarvelSerieComicsUseCase(serie.id, offsetComics).execute().getOrNull()),
    ...
    fun loadCharacters() {
        viewModelScope.launch {
            val jsonResponse = Gson().fromJson<JsonResponse<MarvelCharacter>>(
                Gson().toJson(GetMarvelSerieCharactersUseCase(serie.id,offset).execute().getOrNull()),
